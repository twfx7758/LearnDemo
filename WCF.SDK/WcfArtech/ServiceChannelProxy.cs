    ...
    public class ServiceChannelProxy<TChannel> : RealProxy
    {
        ...
        public override IMessage Invoke(IMessage msg)
        {
            ...
            OperationContractAttribute attribute =
                (OperationContractAttribute)attributes[0];
            string operationName = string.IsNullOrEmpty(attribute.Name) ? methodCall.MethodName : attribute.Name;

            //序列化请求消息
            Message requestMessage = this.MessageFormatters[operationName]
                .SerializeRequest(this.MessageVersion, methodCall.InArgs);
            ...
            //对请求消息进行编码，并将编码生成的字节数组通过HttpWebRequest向服务端发送
            ...
            Message responseMessage = this.MessageEncoderFactory.Encoder
                .ReadMessage(webResponse.GetResponseStream(), int.MaxValue);

            //回复消息进行反序列化生成相应的对象，并映射为方法调用的返回值或者输出/引用参数
            ...
            //通过ReturnMessage的形式将返回值和输出/引用参数返回
