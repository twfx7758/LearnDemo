    ...
    public class ServiceProxyFactory<TChannel>
    {
        ...
        public TChannel CreateChannel()
        {
            MessageEncoderFactory encoderFactory =
                ComponentBuilder.GetMessageEncoderFactory(MessageVersion.Default, Encoding.UTF8);

            ServiceChannelProxy<TChannel> proxy =
                new ServiceChannelProxy<TChannel>(this.Address, MessageVersion.Default, encoderFactory);
            ...
            foreach (OperationDescription operation in contract.Operations)
            {
                IClientMessageFormatter messageFormatter =
                    (IClientMessageFormatter)ComponentBuilder.GetFormatter(operation, true);
